...
$('#step-template')
    .on('click', '.send-request-btn', function(){
        ...
        switch(viewVal[current_step - 1].toUpperCase()){
            case 'RAW':
                request['data'] = $('textarea#attack-request-headers-step'+current_step).val();
                AppSpiderValidate.makeRequest(request);
                break;
            case 'PRETTIFY':
    ...
    .on('click', '.edit-cookies-btn', function(){
        alert('Edit btn clicked in Step '+ current_step);
    })
    .on('click', '.reset-request-btn', function(){
        alert('Reset btn clicked in Step '+ current_step);
    })
