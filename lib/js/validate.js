    ...
    getEncodedHTTPRequest: function() {
        ...
        channel.onMessage.addListener(function(message, sender){
            if (message.fromJS == "background.js" && message.type == "encodedHTTPRequest"){
                ...
                AppSpiderValidate.encodedhttp = encodedHTTPRequest;
                AppSpiderValidate.decodedHeader = AppSpiderValidate.decodeHeader(AppSpiderValidate.encodedhttp);
                AppSpiderValidate.data = AppSpiderValidate.parseHeader(AppSpiderValidate.decodedHeader);
                document.getElementById('appspider-nav').addEventListener('onload', AppSpiderValidate.renderNavTemplate(AppSpiderValidate.data));
                document.getElementById('step-template').addEventListener('onload', AppSpiderValidate.renderTemplate('stepTemplate','step-template', AppSpiderValidate.data));
            }
        });
    }
