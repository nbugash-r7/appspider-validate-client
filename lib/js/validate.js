...
var AppSpiderValidate = {
    ...
    makeRequest: function(request) {
        ...
        //var payload = $('textarea#payload-data').val();
        var attack = {
            'headers': AppSpiderValidate.parseRequest(request),
            'payload': $('textarea#payload-data').val()
        };
    ...
    sendRequest: function(attack) {
        ...
        var protocol = $('#protocol-btn').text().trim() +"://";
        if (a.length == 3) {
            data['method'] = a[0];
            /*TODO: Need to have the ability to switch from http to https*/
            data['url'] = protocol + headers['HOST'] + a[1];
