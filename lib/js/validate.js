    ...
    sendRequest: function(headers) {
        ...
        xhttp.onreadystatechange = function(){
            if (xhttp.readyState == 4 && xhttp.status == 200) {
                console.log('Posting response to the Attack Response section');
        ...
        if (a.length == 3) {
            method = a[0];
            url = headers['Host'] + a[1];
            data['http-version'] = a[2];
        }
        xhttp.open(method, url, true);

        for (var header in headers) {
            if (header != 'Initial-Request-Line' || header != 'Host') {
                if (restrictedChromeHeaders.indexOf(header)){
                    /* For debugging */
                    data['appspider-validate-'+header] = headers[header]
                    xhttp.setRequestHeader('appspider-validate-'+header, headers[header]);
                } else {
                    /* For debugging */
