...
var AppSpiderValidate = {
    ...
    parseHeader: function(requests) {
        ...
        for (var i = 0; i < array_request.length; i++ ) {
            ...
            if (array_request[i].indexOf("#A#B#C#D#E#F#G#H#") > -1) {
                ...
                var headers = AppSpiderValidate.parseRequest(request[0]);
                ...
                var attack_obj = {};
                attack_obj[step_num] = data;
                AppSpiderValidate.saveAttack(attack_obj);
                step_num++;
            }
        }
        return data;
    ...
    sendRequest: function(attack) {
        var data = {};
        var method, url;
        var a = attack.headers['Initial-Request-Line'].split(' ');
    ...
    * */
    ...
    saveAttack: function(attack){
        if (!attack){
            console.log('Error: Invalid Attack!!');
            return;
        }
        var attack_obj = {};
        attack_obj[current_step] = attack;
        chrome.storage.sync.set(attack_obj, function(){
            console.log("Attack saved!!");
        });
    }
};
